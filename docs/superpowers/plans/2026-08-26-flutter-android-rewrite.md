...
- Android application id and namespace: `com.liferpg.app`. `minSdk` is whatever
  `flutter.minSdkVersion` resolves to (24 on Flutter 3.47.1). (Amended 2026-08-26: the plan
  said 23, but Flutter's MinSdkVersionMigration rewrites any below-floor pin to
  `flutter.minSdkVersion` on EVERY build, and FlutterExtension.kt hardcodes 24.)
- Android builds need `compileOnly("org.checkerframework:checker-qual")` added to Android
  subprojects in `android/build.gradle.kts`: firebase-auth's published bytecode carries
  checker-qual type annotations without declaring the dependency, and Kotlin 2.4 errors on
  an inaccessible annotation of an inferred type. Do not remove it.
- For targeted `./gradlew` runs outside `flutter build`, export
  `JAVA_HOME=/usr/lib/jvm/java-17-openjdk-amd64` — bare gradlew picks up JDK 25 and fails.
- Dart package name: `liferpg`.
- Flutter stable channel (verified: 3.47.1), Dart SDK constraint `^3.13.1` as generated by `flutter create` on that toolchain. (Amended 2026-08-26: the plan originally said `>=3.4.0 <4.0.0`, written before the toolchain was known; the resolved Firebase packages require a higher floor.)
